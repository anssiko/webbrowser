...
Flickable {
    ...
    WebView {
        ...
        onLoadFinished: {
            // augment navigator object
            var xhr = new XMLHttpRequest();
            xhr.open("GET", "js/navigator.js");
            xhr.onreadystatechange = function() {
                if (xhr.readyState === xhr.DONE) {
                    viewport.evaluateJavaScript(xhr.responseText);
                    viewport.evaluateJavaScript("navigator.battery.level = " + level + ";")
                    viewport.evaluateJavaScript("navigator.battery.charging = " + charging + ";")
                }
            };
            xhr.send();
        }
